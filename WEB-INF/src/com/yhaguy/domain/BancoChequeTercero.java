...
public class BancoChequeTercero extends Domain {
	...
	public Date getFechaDeposito() throws Exception {
		...
		BancoBoletaDeposito deposito = rr.getBancoDeposito(this.numeroDeposito);
		return deposito != null ? deposito.getFecha() : null;
	}
	...
	public Date getFechaDescuento() throws Exception {
		RegisterDomain rr = RegisterDomain.getInstance();
		if (this.numeroDescuento.contains("REC")) {
			Recibo rec = rr.getRecibo(this.numeroDescuento);
			return rec != null ? rec.getFechaEmision() : null;
		} else {
			String nroDesc = this.numeroDescuento.replace("ANTICIPO", "").replace("PRESTAMO", "");
			if (nroDesc.trim().isEmpty()) {
				return null;
			}
			BancoDescuentoCheque desc = rr.getBancoDescuento(nroDesc);
			return desc != null ? desc.getFecha() : null;
		}		
	}
