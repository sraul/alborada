...
public class RegisterDomain extends Register {
	 ...
	 * @throws Exception
	 */
	public List<CtaCteEmpresaMovimiento> getMovimientosPendientes(IiD empresa,
			Tipo caracterMovimiento) throws Exception {

		String queryMovimientos = "select m from CtaCteEmpresaMovimiento m"
				+ " where m.idEmpresa = ?  and  m.saldo != 0 ";
		...
		List<Object> lista = new ArrayList<Object>();
		List<CtaCteEmpresaMovimiento> listaMovimientosPendientes = new ArrayList<CtaCteEmpresaMovimiento>();
		String whereQuery = "";
		...
		lista = this.hql(queryMovimientos, parametros);
		for (Object o : lista) {
			listaMovimientosPendientes.add((CtaCteEmpresaMovimiento) o);
		}
	...
	public List<PlanDeCuenta> getPlanDeCuentas() throws Exception {
		String query = "select p from PlanDeCuenta p";
	 ...
	 * la lista IiD.
	 */
	public void updateChequesDepositados(List<IiD> cheques,
			String numeroDeposito) throws Exception {
	...
	public void updateChequesDescontados(List<IiD> cheques, String numeroDescuento, boolean prestamo) throws Exception {
	...
	public Deposito getDepositoByFacturaCompraLocal(long idFactura)
			throws Exception {
		String query = "select oc.deposito from CompraLocalOrden oc join oc.facturas f where f.id = "
				+ idFactura;
	 ...
	 */
	public List<NotaCredito> getNotasCreditoVenta(Date desde, Date hasta,
	 ...
	 *         [1]:fecha [2]:numero [3]:cantidad [4]:precio [5]:cliente
	 */
	public List<Object[]> getVentasPorArticulo(long idArticulo, Date desde,
			Date hasta) throws Exception {
	...
	public List<Object[]> getAjustesPorArticulo(long idArticulo, Date desde,
		...
		if (tipo.equals(Configuracion.SIGLA_TM_AJUSTE_POSITIVO)) {
			query = "select a.tipoMovimiento.descripcion, a.fecha, a.numero, d.cantidad, a.sucursal.descripcion, d.costoGs"
	...
	public List<Object[]> getCobranzasPorVendedor(Date desde, Date hasta,
			long idVendedor) throws Exception {
		List<Recibo> cobros = this.getCobranzas(desde, hasta, 0);
	...
	public List<CtaCteEmpresaMovimiento> getCtaCteMovimientos(long idEmpresa,
			Date desde, Date hasta, String caracter) throws Exception {
		String desde_ = misc.dateToString(desde, Misc.YYYY_MM_DD) + " 00:00:00";
		String hasta_ = misc.dateToString(hasta, Misc.YYYY_MM_DD) + " 23:59:00";
	 ...
	 * @return los saldos de clientes/proveedores..
	 */
	public List<CtaCteEmpresaMovimiento> getMovimientosConSaldo(String caracter, long idEmpresa) throws Exception {
		String query = "select c from CtaCteEmpresaMovimiento c where c.anulado = 'FALSE' and c.saldo > 0 and"
				+ " c.tipoCaracterMovimiento.sigla = '" + caracter + "' and c.idEmpresa = " + idEmpresa;
	 ...
	 */
	public List<CtaCteEmpresaMovimiento> getMovimientosConSaldo(Date desde,
			Date hasta, String caracter, long idVendedor, long idEmpresa) throws Exception {
		String desde_ = Utiles.getDateToString(desde, Misc.YYYY_MM_DD) + " 00:00:00";
		String hasta_ = Utiles.getDateToString(hasta, Misc.YYYY_MM_DD) + " 23:59:00";
		String query = "select c from CtaCteEmpresaMovimiento c where c.anulado = 'FALSE' and c.saldo > 0 and"
				+ " c.tipoCaracterMovimiento.sigla = '"
	...
	/**
	 * @return el stock del articulo a una fecha..
	 */
	public long getStock(long idArticulo, Date fecha) throws Exception {
		String fecha_ = Utiles.getDateToString(fecha, Misc.YYYY_MM_DD) + " 23:59:00";
		String query = "select h.stock from HistoricoArticuloStock h where h.idArticulo = " + idArticulo 
	...
	public List<ServicioTecnico> getServiciosTecnicos(String fecha, String numero, String razonSocial, 
			String receptor, String tecnico) throws Exception {
	...
	/**
	 * @return la lista de clientes segun parametros..
	 */
	public List<Cliente> getClientes(String ruc, String razonSocial, String telefono, String direccion) throws Exception {
		String query = "select c from Cliente c where lower(c.empresa.ruc) like '%"
				+ ruc.toLowerCase() + "%' "
				+ " and lower(c.empresa.razonSocial) like '%" + razonSocial.toLowerCase() + "%'"
				+ " and lower(c.empresa.telefono_) like '%" + telefono.toLowerCase() + "%'"
				+ " and lower(c.empresa.direccion_) like '%" + direccion.toLowerCase() + "%'"
				+ " order by c.empresa.razonSocial";
		return this.hqlLimit(query, 50);
	}
	...
	public List<Articulo> getArticulos(String codigoInterno,
		...
		String query = "select a from Articulo a where lower(a.codigoInterno) like '%"
				...
				+ "%' and lower(a.descripcion) like '%"
				+ descripcion.toLowerCase()
				+ "%' and lower(a.articuloFamilia.descripcion) like'%"
				+ familia.toLowerCase() + "%' order by a.codigoInterno";
		return this.hqlLimit(query, 50);
	}
	
	/**
	 * @return los controles de atencion..
	 */
	public List<ServicioControlAtencion> getServicioControlAtenciones(String fecha, boolean agendamiento) throws Exception {
		String query = "select s from ServicioControlAtencion s where"
		+ " cast (s.fecha as string) like '%" + fecha + "%'"
		+ " and s.agendamiento = " + agendamiento
		+ " order by s.fecha";
		return this.hqlLimit(query, 200);
	}
	
	/**
	 * @return los contratos segun paciente..
	 */
	public List<ServicioContrato> getServicioContratos(long idTitular) throws Exception {
		String query = "select s from ServicioContrato s where"
				+ " s.titular.id = " + idTitular
				+ " order by s.fecha";
		return this.hqlLimit(query, 200);
	}
	
	/**
	 * @return el contrato segun numero..
	 */
	public ServicioContrato getServicioContrato(String numero) throws Exception {
		String query = "select s from ServicioContrato s where"
				+ " s.numero = '" + numero + "'";
		List<ServicioContrato> list = this.hql(query);
		return list.size() > 0 ? list.get(0) : null;
	}
	
	/**
	 * @return las cuotas de un contrato..
	 */
	public List<CtaCteEmpresaMovimiento> getServicioContratoCuotas(long idMovimiento) throws Exception {
		String query = "select c from CtaCteEmpresaMovimiento c where c.anulado = 'FALSE' and c.idMovimientoOriginal = " + idMovimiento
				+ " and c.tipoMovimiento.sigla = '" + Configuracion.SIGLA_TM_CONTRATO_SERVICIO_ODONTOLOGIA + "'"
				+ " order by c.fechaVencimiento";
		return this.hql(query);
	}
	
	/**
	 * @return las comisiones..
	 */
	public List<Comisiones> getComisiones(String nombre, String cedula, String cuota, String vencimiento,
			String vendedor, String celular, String particular, String laboral) throws Exception {
		String query = "select c from Comisiones c where upper(c.nombre_apellido) like '%" + nombre.toUpperCase() + "%'"
				+ " and upper(c.cedula) like '%" + cedula.toUpperCase() + "%'"
				+ " and c.fecha_vencimiento like '%" + vencimiento + "%'"
				+ " and upper(c.vendedor) like '%" + vendedor.toUpperCase() + "%'";
		if (!cuota.trim().isEmpty()) {
			query += " and c.nro_cuota = '" + cuota + "'";
		}
		return this.hqlLimit(query, 200);
	}
	
	/**
	 * @return tipos de movimientos que son prestaciones..
	 */
	public List<TipoMovimiento> getPrestaciones() throws Exception {
		List<TipoMovimiento> list = null;
		String queryTipo = "select t from TipoMovimiento t where t.auxi ='" + TipoMovimiento.PRESTACION + "'";
		list = this.hql(queryTipo);
		return list;
	}
	 ...
	 * @return el cobrador..
	 */
	public Cobrador getCobrador(String nombre) throws Exception {
		String query = "select c from Cobrador c where upper(c.nombre) = '" + nombre.toUpperCase() + "'";
		List<Cobrador> list = this.hql(query);
		return list.size() > 0 ? list.get(0) : null;
	}
	 ...
	 * @return el plan..
	 */
	public ServicioPlan getServicioPlan(String descripcion) throws Exception {
		String query = "select p from ServicioPlan p where upper(p.descripcion) = '" + descripcion.toUpperCase() + "'";
		List<ServicioPlan> list = this.hql(query);
		return list.size() > 0 ? list.get(0) : null;
	}
	
	/**
	 * @return el Cliente segun nro_adh..
	 */
	public Cliente getClienteByNroAdherente(String nro_adh) throws Exception {
		String query = "Select c from Cliente c where c.nro_adh = '"+ nro_adh + "'";
		List<Cliente> list = this.hql(query);
		return list.size() > 0 ? list.get(0) : null;
	}
	
	/**
	 * @return las fichas segun cliente..
	 */
	public List<ServicioFicha> getFichas(long idCliente) throws Exception {
		String query = "select s from ServicioFicha s where"
				+ " s.cliente.id = " + idCliente
				+ " order by s.numero";
		return this.hqlLimit(query, 200);
	}
	
	/**
	 * @return la ficha segun numero..
	 */
	public ServicioFicha getServicioFicha(String numero) throws Exception {
		String query = "select s from ServicioFicha s where"
				+ " s.numero = '" + numero + "'";
		List<ServicioFicha> list = this.hql(query);
		return list.size() > 0 ? list.get(0) : null;
	}
	
	/**
	 * @return las cobranzas segun cliente..
	 */
	public List<Cobranza> getCobranzas(long idCliente) throws Exception {
		String query = "select c from Cobranza c where"
	...
	public static void main(String[] args) {
		try {
			String src = "./WEB-INF/docs/process/COMISIONES.csv";
			RegisterDomain rr = RegisterDomain.getInstance();
			
			String[][] cab = { { "Empresa", CSV.STRING } };
			String[][] det = { { "nro_cedula", CSV.STRING }, { "nombres", CSV.STRING }, { "apellidos", CSV.STRING },
					{ "fec_ven", CSV.STRING }, { "nro_cuota", CSV.STRING }, { "importe", CSV.STRING },
					{ "vendedor", CSV.STRING }, { "celular", CSV.STRING }, { "celular2", CSV.STRING },
					{ "tel_particular", CSV.STRING }, { "tel_particular2", CSV.STRING }, { "tel_laboral", CSV.STRING },
					{ "tel_laboral2", CSV.STRING }};
			...
			while (csv.hashNext()) {
				...
				String cedula = csv.getDetalleString("nro_cedula");	
				String nombre = csv.getDetalleString("nombres");
				String apellido = csv.getDetalleString("apellidos");
				String vto = csv.getDetalleString("fec_ven");
				String cuota = csv.getDetalleString("nro_cuota");
				String importe = csv.getDetalleString("importe");
				String vendedor = csv.getDetalleString("vendedor");
				String cel = csv.getDetalleString("celular");
				String cel2 = csv.getDetalleString("celular2");
				String part = csv.getDetalleString("tel_particular");
				String part2 = csv.getDetalleString("tel_particular2");
				String lab = csv.getDetalleString("tel_laboral");
				String lab2 = csv.getDetalleString("tel_laboral2");
				
				Comisiones com = new Comisiones();
				com.setCedula(cedula);
				com.setNombre_apellido(nombre + " " + apellido);
				com.setFecha_vencimiento(vto);
				com.setNro_cuota(cuota);
				com.setImporte(Double.parseDouble(importe));
				com.setVendedor(vendedor);
				com.setCelular(cel);
				com.setCelular_2(cel2);
				com.setTel_particular(part);
				com.setTel_particular_2(part2);
				com.setTel_laboral(lab);
				com.setTel_laboral_2(lab2);
				
				rr.saveObject(com, "sys");
