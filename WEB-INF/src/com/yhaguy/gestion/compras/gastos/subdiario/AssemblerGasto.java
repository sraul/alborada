...
public class AssemblerGasto extends Assembler {
	...
	final static String[] ATT_CTA_CTE_MOVIMIENTO = { "nroComprobante", "idEmpresa", "importeOriginal", "idEmpresa" };
	...
	public Domain dtoToDomain(DTO dtoG) throws Exception {
		...
		this.listaDTOToListaDomain(dto, domain, "detalles", true, true,
				new AssemblerGastoDetalle());		
		this.listaDTOToListaDomain(dto, domain, "formasPago", true, true,
				new AssemblerReciboFormaPago(dto.getNumeroFactura()));
		this.listaMyArrayToListaDomain(dto, domain, "aplicacionesDebitoBancario");
	...
	public DTO domainToDto(Domain domain) throws Exception {
		...
		this.listaDomainToListaDTO(domain, dto, "formasPago",
				new AssemblerReciboFormaPago(dto.getNumeroFactura()));
		this.listaDomainToListaMyArray(domain, dto, "aplicacionesDebitoBancario", ATT_CTA_CTE_MOVIMIENTO);
		
		for (MyArray deb : dto.getAplicacionesDebitoBancario()) {
			long idEmpresa = (long) deb.getPos4();
			String razonSocial = (String) rr.getDatosEmpresa(idEmpresa)[0];
			deb.setPos2(razonSocial);
		}
