...
public class InformeHechauka {
	...
	public static synchronized void generarInformeHechauka(List<Venta> ventas)
		...
		for (Venta venta : ventas) {
			...
			String rSocial = venta.getDenominacion() == null ? venta
					.getCliente().getRazonSocial() : venta.getDenominacion();
			String col5 = "1";
			String nro = venta.getNumero();
			String fecha = misc.dateToString(venta.getFecha(), Misc.DD_MM_YYYY)
					.replace("-", "/");
			double importe = redondear(venta.getTotalImporteGs());
			double iva10 = redondear(misc.calcularIVA(importe, 10));
			double gravada = importe - iva10;
			long col10 = 0;
			long col11 = 0;
			long col12 = 0;
			long col14 = venta.getCondicionPago().getId();
			long col15 = 0;
			long col16 = 0;
		...
		saveArchivo(
				objects,
				getCabecera(registros, montoTotal),
				"HechaukaVentas-"
	 ...
	 * Generar informe Hechauka..
	 */
	public static synchronized void generarInformeHechaukaNotaCred(
			List<NotaCredito> ncs) throws Exception {
		Misc misc = new Misc();
		List<String> objects = new ArrayList<String>();
		...
		for (NotaCredito nc : ncs) {
			...
			String fecha = misc.dateToString(nc.getFechaEmision(),
					Misc.DD_MM_YYYY).replace("-", "/");
	 ...
	 */
	private static String getCabecera(int registros, double montoTotal) {
		String out = "";
