...
import com.yhaguy.gestion.articulos.BeanArticuloHistorial;
...
public class ReportesViewModel extends SimpleViewModel {
	...
	static final MyArray TESORERIA = new MyArray("Tesorería", ID_TESORERIA);
	static final MyArray COMPRAS = new MyArray("Compras", ID_COMPRAS);
	static final MyArray VENTAS = new MyArray("Ventas", ID_VENTAS);
	static final MyArray STOCK = new MyArray("Stock", ID_STOCK);
	static final MyArray LOGISTICA = new MyArray("Logística", ID_LOGISTICA);
	static final MyArray CONTABILIDAD = new MyArray("Contabilidad", ID_CONTABILIDAD);
	static final MyArray RRHH = new MyArray("Recursos Humanos", ID_RRHH);
	static final MyArray SISTEMA = new MyArray("Sistema", ID_SISTEMA);
	...
	class ReportesDeStock {
		...
		private void historialMovimientosArticulo() {
			try {
				...
				List<Object[]> ajustStockPost = rr.getAjustesPorArticulo(idArticulo, desde, hasta, idSucursal, Configuracion.SIGLA_TM_AJUSTE_POSITIVO);
				List<Object[]> ajustStockNeg = rr.getAjustesPorArticulo(idArticulo, desde, hasta, idSucursal, Configuracion.SIGLA_TM_AJUSTE_NEGATIVO);
				...
				String source = com.yhaguy.gestion.reportes.formularios.ReportesViewModel.SOURCE_HISTORIAL_MOVIMIENTOS_ART;
				Map<String, Object> params = new HashMap<String, Object>();
				JRDataSource dataSource = new HistorialMovimientosArticuloDataSource(
						historico, desde, hasta, articulo.getCodigoInterno() + " - " + articulo.getDescripcion());
				params.put("Usuario", getUs().getNombre());
				imprimirJasper(source, params, dataSource, formato);
	...
	class ReportesVentas {
		...
		public ReportesVentas(String codigoReporte, boolean mobile) {
			...
			case VENTAS_RANKING_POR_FLIA_DET:
				this.rankingPorFamiliaDetallado(mobile);
				break;
		...
		public void cobranzasPorVendedor(boolean mobile) {
			try {
				...
				List<Object[]> cobranzas = rr.getCobranzasPorVendedor(desde,
						hasta, vendedor.getId());
				...
				for (NotaCredito notacred : ncs) {
					if ((!notacred.isAnulado()) && notacred.isMotivoDevolucion()) {
						...
						Object[] nc = new Object[] {
								...
								notacred.isNotaCreditoVentaContado() ? "NC-CO "
										+ motivo : "NC-CR " + motivo,
								notacred.getTotalCostoGsSinIva() * -1};					
						data.add(nc);
					}
				}
				...
				for (Venta venta : ventas) {
					if ((!venta.isAnulado())) {
						Object[] vta = new Object[] {
								...
								venta.getNumero(),
								"FAC. " + venta.getCondicionPago().getDescripcion().substring(0, 3).toUpperCase(),
								venta.getTotalCostoGsSinIva() };						
						data.add(vta);
					}
				}
				String sucursal = getAcceso().getSucursalOperativa().getText();

				ReporteCostoVentas rep = new ReporteCostoVentas(desde, hasta, sucursal);
				rep.setDatosReporte(data);
		 ...
		 * VEN-00013
		 */
		private void rankingPorFamiliaDetallado(boolean mobile) {
			if (mobile) {
			...
			try {
				...
				String familia_ = familia == null? "TODOS.." : familia.getDescripcion();				
				String source = com.yhaguy.gestion.reportes.formularios.ReportesViewModel.SOURCE_RANKING_VTAS_FLIA_DET;
				Map<String, Object> params = new HashMap<String, Object>();
			...
			try {
				...
				for (Venta venta : vtas) {
					if (!venta.isAnulado()) {
						...
						if (acum != null) {
							importeGs = importeGs + (double) acum[0];
							costoGs = costoGs + (double) acum[1];
						}							
				...
				for (Cliente cliente : clientes) {
					...
					data.add(new Object[] { (int) 0, cliente.getRuc(),
							cliente.getRazonSocial(),
							(double)acum[0], (double)acum[1] });
				}
				...
				ReporteRankingDeClientes rep = new ReporteRankingDeClientes(
						desde, hasta, getAcceso().getSucursalOperativa().getText());
				rep.setDatosReporte(data);
				rep.setApaisada();
		...
		private void totalCobranzasPorVendedor(boolean mobile) {
			try {
				...
				RegisterDomain rr = RegisterDomain.getInstance();
				List<Object[]> data = new ArrayList<Object[]>();
				List<Object[]> cobros = rr.getCobranzasPorVendedor(desde, hasta, 0);
					...
					for (NotaCredito ncred : ncs) {
						...
						if (vendedor == null || vendedor.getId().longValue() == ncred.getVendedor().getId().longValue()) {
							if (rubro == null || rubro.getDescripcion().equals(ncred.getCliente().getRubro())) {
								if ((!ncred.isAnulado()) && (!ncred.isMotivoReclamo())) {
	...
	class ReportesTesoreria {
		...
		private void saldosCtaCteDetallado(boolean cliente, boolean mobile) throws Exception {
			...
			List<CtaCteEmpresaMovimiento> movims = rr.getMovimientosConSaldo(desde_, hasta_, caracter, idVendedor, idEmpresa);	
			List<CtaCteEmpresaMovimiento> movims_ = rr.getMovimientosConSaldo(desde_, hasta_, caracter, idVendedor, idEmpresa);
			...
			if (!fraccionado) {
				...
				for (CtaCteEmpresaMovimiento movim : movims_) {
					if (rubro != null && movim.getEmpresa().isRubro(rubro.getId())
							|| rubro == null) {
			...
			Map<String, Object> params = new HashMap<String, Object>();
			JRDataSource dataSource = new CtaCteSaldosDataSource(rubro != null ? aux : movims, fraccionado ? movims : aux);
		...
		private void chequesDescontados(boolean mobile) throws Exception {
			...
			for (BancoChequeTercero cheque : cheques) {
				if (!cheque.getNumeroDescuento().equals("0")) {
					BancoDescuentoCheque desc = rr.getBancoDescuento(cheque.getNumeroDescuento());
					Date fechaDesc = desc != null ? desc.getFecha() : null;
					data.add(new Object[] {
							Utiles.getDateToString(fechaDesc, Utiles.DD_MM_YYYY),
		...
		private void clientesConMovimientos(boolean mobile) throws Exception {
			...
			for (HistoricoMovimientos movim : movims) {
				Cliente cli = rr.getClienteById(movim.getIdCliente());
				data.add(new Object[] { cli.getRazonSocial(),
						...
						cli.getLimiteCredito(),						
						cli.isVentaCredito() ? "SI" : "NO"});
			}
		...
		private void matrizImportaciones() {
			try {
				...
				for (ImportacionPedidoCompra imp : imps) {
					int mes = Utiles.getNumeroMes(imp.getFechaCreacion()) - 1;
					
					for (ImportacionPedidoCompraDetalle item : imp.getImportacionPedidoCompraDetalle()) {
						String cod = item.getArticulo().getCodigoInterno();
						String key = cod + "-" + mes;
						Object[] acum = values.get(key);
						Object[] datos = rr.getStockCosto(item.getArticulo().getId());
	...
	class ReportesDeContabilidad {
		...
		static final String PLANILLA_CIERRE = "CON-00006";
		...
		static final String SALDO_CLIENTES_PERIODO = "CON-00010";
		...
		static final String SALDO_PROVEEDORES_PERIODO = "CON-00016";
		...
		public ReportesDeContabilidad(String codigoReporte, boolean mobile) throws Exception {
			...
			case SALDO_CLIENTES_PERIODO:
				this.saldosCtaCtePeriodo(true, false);
				break;
				
			case STOCK_MERCADERIA_COSTO_PROMEDIO:
				this.stockMercaderiaCostoPromedio();
				break;	
			...
			case SALDO_PROVEEDORES_PERIODO:
				this.saldosCtaCtePeriodo(false, false);
				break;
		...
		private void libroVentas() throws Exception {
			...
			if (formularioContinuo)
				source = com.yhaguy.gestion.reportes.formularios.ReportesViewModel.SOURCE_LIBRO_VENTAS_FC;
			Map<String, Object> params = new HashMap<String, Object>();
		...
		private void planillasCaja() {
			...
			try {
				RegisterDomain rr = RegisterDomain.getInstance();
				List<CajaPeriodo> planillas = rr.getCajaPlanillas(desde, hasta);
				
				String source = com.yhaguy.gestion.reportes.formularios.ReportesViewModel.SOURCE_RESUMEN;
				Map<String, Object> params = new HashMap<String, Object>();
				JRDataSource dataSource = new PlanillasCajaDataSource(planillas);
				params.put("Usuario", getUs().getNombre());
				params.put("Periodo",Utiles.getDateToString(desde, Misc.DD_MM_YYYY)
								+ " AL "
								+ Utiles.getDateToString(hasta, Misc.DD_MM_YYYY));
				params.put("NroPlanilla", "- - -");
				params.put("Cajero", "- - -");
				imprimirJasper(source, params, dataSource, formato);
		...
		private void chequesDescontadosAvencer() throws Exception {
			try {
				...
				Date emisionHasta = Utiles.getFecha("31-12-" + emision + " 00:00:00");
				Date vtoDesde = Utiles.getFecha("01-01-" + vencimiento + " 00:00:00");
				Date vtoHasta = Utiles.getFecha("31-12-" + vencimiento + " 00:00:00");
		...
		/**
		 * reporte CON-00010 / CON-00016 / CON-00018
		 */
		private void saldosCtaCtePeriodo(boolean cliente, boolean aseguradora) throws Exception {
			
			String periodo = filtro.getAnhoDesde();
			Object[] formato = filtro.getFormato();
			
			if (periodo == null) {
				Clients.showNotification("Debe especificar el periodo..", 
						Clients.NOTIFICATION_TYPE_ERROR, null, null, 0);
				return;
			}
			
			RegisterDomain rr = RegisterDomain.getInstance();
			Date desde = Utiles.getFecha("01-01-2015 00:00:00");			
			Date hasta = new Date();			
			...
			case Utiles.ANHO_2016:
				hasta = Utiles.getFecha("31-12-2016 00:00:00");
				List<CtaCteEmpresaMovimiento_2016> movims = rr.getMovimientosConSaldo2016(desde, hasta, caracter, 0);
				List<CtaCteEmpresaMovimiento_2016> movims_ = rr.getMovimientosConSaldo2016(desde, hasta, caracter, 0);
				List<CtaCteEmpresaMovimiento_2016> aux = new ArrayList<CtaCteEmpresaMovimiento_2016>();
				Map<Long, CtaCteEmpresaMovimiento_2016> acum = new HashMap<Long, CtaCteEmpresaMovimiento_2016>();
				
				for (CtaCteEmpresaMovimiento_2016 movim : movims_) {
					if (movim.isVentaCredito()) {
						CtaCteEmpresaMovimiento_2016 cmovim = acum.get(movim.getIdMovimientoOriginal());
						if (cmovim != null) {
							cmovim.setSaldo(cmovim.getSaldo() + movim.getSaldo());
							int cmp = movim.getFechaVencimiento().compareTo(cmovim.getFechaVencimiento());
							if (cmp < 0) {
				...
				// ordena la lista segun fecha..
				Collections.sort(aux, new Comparator<CtaCteEmpresaMovimiento_2016>() {
					@Override
					public int compare(CtaCteEmpresaMovimiento_2016 o1,
							CtaCteEmpresaMovimiento_2016 o2) {
						Date fecha1 = o1.getFechaEmision();
						Date fecha2 = o2.getFechaEmision();
						return fecha1.compareTo(fecha2);
					}
				...
				String source = com.yhaguy.gestion.reportes.formularios.ReportesViewModel.SOURCE_SALDO_DET;
				Map<String, Object> params = new HashMap<String, Object>();
				JRDataSource dataSource = new CtaCteSaldosDataSource_2016(movims, aux, aseguradora);
				params.put("Titulo", cliente ? "SALDOS DE CLIENTES DETALLADO PERIODO " + periodo 
						: "SALDOS DE PROVEEDORES DETALLADO PERIODO " + periodo);
				params.put("Usuario", getUs().getNombre());
				params.put("Vendedor", "TODOS..");
			...
			case Utiles.ANHO_2017:
				hasta = Utiles.getFecha("31-12-2017 00:00:00");
				List<CtaCteEmpresaMovimiento> movims17 = rr.getMovimientosConSaldo(desde, hasta, caracter, 0);
				List<CtaCteEmpresaMovimiento> movims_17 = rr.getMovimientosConSaldo(desde, hasta, caracter, 0);
				List<CtaCteEmpresaMovimiento> aux17 = new ArrayList<CtaCteEmpresaMovimiento>();
				Map<Long, CtaCteEmpresaMovimiento> acum17 = new HashMap<Long, CtaCteEmpresaMovimiento>();
				
				for (CtaCteEmpresaMovimiento movim : movims_17) {
					if (movim.isVentaCredito()) {
						CtaCteEmpresaMovimiento cmovim = acum17.get(movim.getIdMovimientoOriginal());
						if (cmovim != null) {
							cmovim.setSaldo(cmovim.getSaldo() + movim.getSaldo());
							int cmp = movim.getFechaVencimiento().compareTo(cmovim.getFechaVencimiento());
							if (cmp < 0) {
							...
							acum17.put(movim.getIdMovimientoOriginal(), cmovim);
						} else {
							acum17.put(movim.getIdMovimientoOriginal(), movim);
						}
					} else {
						aux17.add(movim);
					}
				}
				for (Long key : acum17.keySet()) {
					aux17.add(acum17.get(key));
				}
				// ordena la lista segun fecha..
				Collections.sort(aux17, new Comparator<CtaCteEmpresaMovimiento>() {
					@Override
					public int compare(CtaCteEmpresaMovimiento o1,
							CtaCteEmpresaMovimiento o2) {
						Date fecha1 = o1.getFechaEmision();
						Date fecha2 = o2.getFechaEmision();
						return fecha1.compareTo(fecha2);
					}
				...
				String source17 = com.yhaguy.gestion.reportes.formularios.ReportesViewModel.SOURCE_SALDO_DET;
				Map<String, Object> params17 = new HashMap<String, Object>();
				JRDataSource dataSource17 = new CtaCteSaldosDataSource(movims17, aux17);
				params17.put("Titulo", cliente ? "SALDOS DE CLIENTES DETALLADO PERIODO " + periodo 
						: "SALDOS DE PROVEEDORES DETALLADO PERIODO " + periodo);
				params17.put("Usuario", getUs().getNombre());
				params17.put("Vendedor", "TODOS..");
				imprimirJasper(source17, params17, dataSource17, formato);
				
				break;
				
				default:
					Clients.showNotification("Periodo incorrecto..", 
							Clients.NOTIFICATION_TYPE_ERROR, null, null, 0);					
				break;
			}				
		}
		 ...
		 * reporte CON-00011
		 */
		private void stockMercaderiaCostoPromedio() throws Exception {
			try {
				Date hasta = filtro.getFechaHasta();
				Articulo art = filtro.getArticulo();
				...
				if (art == null) {
					arts = rr.getArticulos_();
					for (Articulo item : arts) {
						if (!item.getCodigoInterno().startsWith("@")) {
							long stock = rr.getStock(item.getId(), hasta);
							double costoPromedio = rr.getCostoPromedio(item.getId(), hasta);
							data.add(new Object[] { item.getCodigoInterno(), item.getDescripcion(), 
									stock, costoPromedio, (stock * costoPromedio) });	
						}
					}
				} else {
					long stock = rr.getStock(art.getId(), hasta);
					data.add(new Object[] { art.getCodigoInterno(), art.getDescripcion(), stock});
				}
				
				String desc = art == null ? "TODOS.." : art.getCodigoInterno();
				ReporteStockCostoPromedio rep = new ReporteStockCostoPromedio(hasta, desc);
				rep.setApaisada();
				rep.setDatosReporte(data);
...
class ReporteCostoVentas extends ReporteYhaguy {
	...
	public ReporteCostoVentas(Date desde, Date hasta, String sucursal) {
		this.desde = desde;
		this.hasta = hasta;
		this.sucursal = sucursal;
	}
...
class LibroVentasDataSource implements JRDataSource {
	...
	private void loadDatos() {
		for (NotaCredito ncred : this.notasCredito) {
			...
			String grav5 = "";
			String grav10 = ncred.isAnulado() ? "" : FORMATTER.format(ncred
					.getTotalGravado10() * -1);
			String iva5 = "";
			String iva10 = ncred.isAnulado() ? "" : FORMATTER.format(ncred
					.getTotalIva10() * -1);
			String total = ncred.isAnulado() ? "" : FORMATTER.format(ncred
					.getImporteGs() * -1);
		...
		for (Venta vta : this.ventas) {
			...
			String concepto = TipoMovimiento.getAbreviatura(vta
					.getTipoMovimiento().getSigla());
			String numero = vta.getNumero();
			String razonSocial = vta.isAnulado() ? "ANULADO" : vta
					.getDenominacion();
			if (razonSocial == null)
			...
			if (ruc.isEmpty())
				ruc = Configuracion.RUC_EMPRESA_LOCAL;
			String grav5 = "";
			String grav10 = vta.isAnulado() ? "" : FORMATTER.format(vta
					.getTotalGravado10());
			String iva5 = "";
			String iva10 = vta.isAnulado() ? "" : FORMATTER.format(vta
					.getTotalIva10());
			String total = vta.isAnulado() ? "" : FORMATTER.format(vta
					.getTotalImporteGs());
			values.add(new BeanLibroVenta(fecha, concepto, numero, razonSocial,
					vta.isAnulado() ? "" : ruc, grav10, iva10, grav5, iva5,
					total));
...
/**
 * DataSource del historial de movimientos del articulo..
 */
class HistorialMovimientosArticuloDataSource implements JRDataSource {
	...
	public HistorialMovimientosArticuloDataSource(List<Object[]> historial,
			Date desde, Date hasta, String articulo) {
	...
	/**
	 * [0]:concepto [1]:fecha [2]:numero [3]:cantidad [4]:precio [5]:cliente
	 */
	@Override
	public Object getFieldValue(JRField field) throws JRException {
		Object value = null;
		...
		if ("Historial".equals(fieldName)) {
			if ((this.historial.size()) < this.currentSize) {
				this.currentSize = this.historial.size();
			}

			List<BeanArticuloHistorial> values = new ArrayList<BeanArticuloHistorial>();
			List<Object[]> temp = new ArrayList<Object[]>();
			for (int i = 0; i < this.currentSize; i++) {
				Object[] hist = this.historial.get(i);
				boolean ent = ((String) hist[0]).startsWith("(+)");
				String fecha = misc.dateToString((Date) hist[1],
						Misc.DD_MM_YYYY);
				String numero = (String) hist[2];
				String concepto = ((String) hist[0]).replace("(+)", "");
				String entrada = ent ? FORMATTER.format(hist[3]) : "";
				String salida = ent ? "" : FORMATTER.format(hist[3]);
				String importe = FORMATTER.format(hist[4]);
				temp.add(hist);
				values.add(new BeanArticuloHistorial(fecha, numero, concepto,
						entrada, salida, importe));
				this.totalImporte += ((double) hist[4]);
			}
			value = values;
			this.historial.removeAll(temp);

		} else if ("Desde".equals(fieldName)) {
			value = misc.dateToString(this.desde, Misc.DD_MM_YYYY);
		} else if ("Hasta".equals(fieldName)) {
			value = misc.dateToString(this.hasta, Misc.DD_MM_YYYY);
		} else if ("Articulo".equals(fieldName)) {
			value = this.articulo;
		}
		return value;
	}
 ...
 */
class ReporteStockCostoPromedio extends ReporteYhaguy {
	
	private Date hasta;
	private String articulo;
	
	public ReporteStockCostoPromedio(Date hasta, String articulo) {
		this.hasta = hasta;
		this.articulo = articulo;
	}
	...
	static DatosColumnas col2 = new DatosColumnas("Descripción", TIPO_STRING);
	static DatosColumnas col3 = new DatosColumnas("Stock", TIPO_LONG, 20, true);
	static DatosColumnas col4 = new DatosColumnas("Costo Promedio Gs.", TIPO_DOUBLE_GS, 40, true);
	static DatosColumnas col5 = new DatosColumnas("Total Promedio Gs.", TIPO_DOUBLE_GS, 40, true);
	...
	@Override
	public void informacionReporte() {
		this.setTitulo("Stock al costo promedio (a una fecha dada)");
		this.setDirectorio("Articulos");
		this.setNombreArchivo("stock-");
		this.setTitulosColumnas(cols);
		this.setBody(this.getCuerpo());
