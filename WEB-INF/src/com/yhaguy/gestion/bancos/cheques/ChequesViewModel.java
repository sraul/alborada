...
public class ChequesViewModel extends SimpleViewModel {
	...
	private void imprimirCheque(MyArray cheque) {
		...
		String source = ReportesViewModel.SOURCE_CHEQUE_AL_DIA;
		if (!alDia) {
			source = ReportesViewModel.SOURCE_CHEQUE_DIFERIDO;
		}
		Map<String, Object> params = new HashMap<String, Object>();
		JRDataSource dataSource = new ChequeDataSource(cheque);
	...
	public List<MyArray> getCheques() throws Exception {
		...
		List<BancoCheque> cheques = rr.getCheques(this.filterNro, this.filterBanco,
				...
				this.filterNumeroOrdenPago, aVencer, pendienteCobro, null, null);
	...
	private void reporteCheques() throws Exception {
		...
		for (MyArray cheque : this.getCheques()) {
			data.add(new Object[] {
					Utiles.getDateToString((Date) cheque.getPos4(),
							Utiles.DD_MM_YYYY), cheque.getPos3() + "",
