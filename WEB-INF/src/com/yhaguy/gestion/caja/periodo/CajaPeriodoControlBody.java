...
public class CajaPeriodoControlBody extends BodyApp {
	...
	private void saveRecibo() throws Exception {
		...
		if (this.reciboDTO.isOrdenPagoGastosContado()) {
	...
	private boolean isStockDisponible(VentaDTO pedido) throws Exception {
		...
		for (VentaDetalleDTO item : pedido.getDetalles()) {
			long stock = rr.getStockDisponible(item.getArticulo().getId(), Deposito.ID_DEPOSITO_PRINCIPAL);
	...
	public void devolucionConNotaDeCredito() throws Exception {
		...
		if (b.isClickAceptar()) {
			...
			NotaCreditoDTO notaCred = ctr.crearNotaCreditoVentaDesde(nc,
					getDtoUtil().getEstadoComprobanteCerrado());
	...
	private void imprimirVenta(VentaDTO venta) throws Exception {
		...
		String source = ReportesViewModel.SOURCE_VENTA;
		if (Configuracion.empresa.equals(Configuracion.EMPRESA_BATERIAS)) {
			source = ReportesViewModel.SOURCE_VENTA_BATERIAS;
		}
		
		JRDataSource dataSource = new VentaDataSource(venta);
		String vencimiento = this.m.dateToString(venta.getVencimiento(),
				Misc.DD_MM_YYYY);
	...
	private void imprimirNotaCredito() throws Exception {
		...
		Date fechaFac = (Date) nc.getDetallesFacturas().get(0).getVenta()
				.getPos3();
		String fechaFac_ = this.m.dateToString(fechaFac, Misc.DD_MM_YYYY);
		String condicion = (String) nc.getDetallesFacturas().get(0).getVenta()
				.getPos6();
		...
		Map<String, Object> params = new HashMap<String, Object>();
		JRDataSource dataSource = new NotaCreditoDataSource();
		...
		params.put("NroFactura", nc.getDetallesFacturas().get(0).getVenta()
				.getPos2());
	...
	/**
	 * DataSource de la Venta..
	 */
	class VentaDataSource implements JRDataSource {
	...
	/**
	 * DataSource de la Nota de Credito..
	 */
	class NotaCreditoDataSource implements JRDataSource {

		NotaCreditoDTO notacredito;

		public NotaCreditoDataSource() {
			this.notacredito = selectedNotaCredito;
		}
		...
		private NotaCreditoDetalleDTO getItemDescuento() {
			...
			NotaCreditoDetalleDTO out = new NotaCreditoDetalleDTO(getIva10());
			out.setArticulo(art);
			out.setCantidad(1);
