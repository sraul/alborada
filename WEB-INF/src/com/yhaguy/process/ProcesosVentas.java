...
public class ProcesosVentas {
	 ...
	 */
	public static void addHistoricoComisiones(Date desde, Date hasta) {
		try {			
			Misc m = new Misc();
			RegisterDomain rr = RegisterDomain.getInstance();
			...
			cobros = rr.getCobranzasPorVendedor(desde, hasta, 0);
			...
			for (Object[] cobro : cobros) {
				...
				for (Proveedor prov : proveedores) {
					...
					if (nc != null && nc.isMotivoDescuento()) {
						int cantidad = det.getVenta().getCantidadItemsByProveedor(prov.getId());
						double monto = (nc.getImporteGs() / det.getVenta().getDetalles().size()) * cantidad;
						importeNcr = (monto);
						importeDescuento = nc.getImporteGs();
					}
					
					porc = Utiles.obtenerPorcentajeDelValor(importeProv, (importeVenta - importeDescuento));
	...
	public static void main(String[] args) {
		try {
			...
			ProcesosVentas.addHistoricoComisiones(Utiles.getFecha("01-03-2017 00:00:00"), Utiles.getFecha("31-03-2017 23:00:00"));
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
