...
import com.yhaguy.domain.HistoricoArticuloStock;
...
public class ProcesosArticulos {
	...
	public static void recalcularStock(long idDeposito, long idSucursal) throws Exception {		
		...
		for (ArticuloDeposito adp : adps) {
			...
			if (!adp.getArticulo().getCodigoInterno().startsWith("@")) {	
				...
				List<Object[]> ventas = rr.getVentasPorArticulo(
						idArticulo, desde_, hasta_);
				List<Object[]> ntcsv = rr.getNotasCreditoVtaPorArticulo(
						idArticulo, desde_, hasta_);
				List<Object[]> ntcsc = rr.getNotasCreditoCompraPorArticulo(
						idArticulo, desde_, hasta_);
				List<Object[]> compras = rr.getComprasLocalesPorArticulo(
						idArticulo, desde_, hasta_);
				List<Object[]> importaciones = rr.getComprasImportacionPorArticulo(
						idArticulo, desde_, hasta_);
				List<Object[]> transfs = rr.getTransferenciasPorArticulo(
						idArticulo, desde_, hasta_);
				List<Object[]> ajustStockPost = rr.getAjustesPorArticulo(
						idArticulo, desde_, hasta_, idSucursal,
						Configuracion.SIGLA_TM_AJUSTE_POSITIVO);
				List<Object[]> ajustStockNeg = rr.getAjustesPorArticulo(
						idArticulo, desde_, hasta_, idSucursal,
						Configuracion.SIGLA_TM_AJUSTE_NEGATIVO);
				List<Object[]> migracion = rr.getMigracionPorArticulo(adp.getArticulo().getCodigoInterno(), desde_, hasta_,
						idSucursal);
	...
	*/}
	...
	public static void poblarHistoricoStock() throws Exception {
		...
		Date hasta = Utiles.getFecha("14-03-2017 00:00:00");

		Calendar start = Calendar.getInstance();
		start.setTime(Utiles.getFecha("05-03-2017 00:00:00"));
		...
		for (Date date = start.getTime(); start.before(end); start.add(Calendar.DATE, 1), date = start.getTime()) {
			for (Articulo art : arts) {
				if (!art.getCodigoInterno().startsWith("@")) {
					...
					HistoricoArticuloStock historico = new HistoricoArticuloStock();
					historico.setFecha(date);
					historico.setIdArticulo(art.getId());
					historico.setStock(stockEntrada - stockSalida);
					historico.setCostoPromedioGs(ControlArticuloCosto.getCostoPromedio(art.getId(), date));
					rr.saveObject(historico, "process");
					System.out.println(Utiles.getDateToString(date, Utiles.DD_MM_YY) 
							+ " stock: " + historico.getStock() + " - " + art.getCodigoInterno());
				}
			}		    
		}		
	}
